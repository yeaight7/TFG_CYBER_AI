...
The committed Phase-2 input (`pcaps/lab_capture_traffic.csv`, ~2M flow rows) is **real captured packet traffic from
a closed, operator-controlled lab** — not real-world traffic and not
algorithmically synthesised. Its labels (`truth_label` / `truth_y` /
`source_label`) are **generator-intent** labels supplied by the operator, not an
independent detector's verdict, so a high Phase-2 score is an *in-/near-distribution
controlled-lab check with limited external validity*, not real-world evidence.

The traffic was generated by hand (console commands) by the operator in the home
lab; the earlier GCP/Docker generator (`lab/docker/`, `gen_traffic.py`) was a
**deprecated** attempt whose flows were unusable and is **not** the source of the
committed CSV. Full provenance is recorded in
[`pcaps/README.md`](../pcaps/README.md). Treat the generic "Execution Steps"
below as a **reference workflow**, not a transcript of the exact commands used.
