...
\texttt{fp~=~\textminus2.0}, \texttt{fn~=~\textminus5.0}, \texttt{omission~=~0.0}. Su \texttt{metrics.json} recoge
...
\texttt{fp~=~\textminus1.5} como normalización vigente, pero el mejor artefacto histórico usó
\texttt{fp~=~\textminus2.0}. Si el tribunal te pregunta sobre los rewards que usaba tu mejor
modelo, no contestes con los defaults actuales; contesta con los del artefacto concreto. Y si te
preguntan sobre lo que usa hoy el código, entonces sí: \texttt{tp=1.5}, \texttt{fp=\textminus1.5},
\texttt{fn=\textminus5.0}, \texttt{omission=0.0}. Esa distinción entre \textbf{estado actual del
código} y \textbf{estado
...
Optimiza una asimetría de seguridad: castiga mucho más dejar pasar ataques (\texttt{fn~=~\textminus5.0}) que
bloquear benignos (\texttt{fp~=~\textminus1.5}), recompensa bloquear ataques (\texttt{tp~=~1.5}) y deja
