...
- **Los resultados históricos no coinciden necesariamente con los defaults actuales del código**. El propio repositorio lo advierte: el reward actual por defecto usa `fp=-1.5`, mientras que el mejor run histórico consolidado usó `fp=-2.0`. Lo mismo ocurre con otros hiperparámetros históricos. En tu memoria conviene separar con nitidez “**defaults del código actual**” frente a “**settings del artefacto reportado**”.
