...
Este capítulo describe el diseño metodológico empleado para construir y evaluar el defensor de ciberseguridad basado en aprendizaje por refuerzo (RL, del inglés \emph{Reinforcement Learning}) propuesto. El objetivo no es presentar un sistema de prevención de intrusiones (IPS, del inglés \emph{Intrusion Prevention System}) listo para producción, sino definir un marco experimental controlado, reproducible y fuera de línea (\emph{offline}) para decisiones de seguridad binarias sobre registros de flujos de red. Cada flujo se trata como un único punto de decisión: el defensor observa una representación numérica del flujo y decide si permitirlo (\texttt{PERMIT}) como tráfico benigno o bloquearlo (\texttt{BLOCK}) como tráfico malicioso.
...
\begin{enumerate}
    ...
    \item conservar la representación canónica sin escalar en una caché validada, construir la partición y ajustar el escalador solo con entrenamiento;
    \item entrenar y evaluar un defensor QRDQN compatible con Gymnasium bajo protocolos controlados, compararlo con una línea base supervisada y aplicar la inferencia offline de Fase~2.
\end{enumerate}
...
La Figura~\ref{fig:pipeline-sistema} resume esta arquitectura en dos recorridos separados: entrenamiento y evaluación sobre CICIDS2017, e inferencia offline posterior sobre tráfico de laboratorio. La separación visual refuerza una idea metodológica central: la Fase 2 reutiliza el esquema canónico, el escalador y el modelo entrenado, pero no reentrena ni reajusta el pipeline sobre la captura de laboratorio.

\begin{figure}[htbp]
\centering
\resizebox{\textwidth}{!}{\input{figuras/f2_pipeline_sistema}}
\caption[Pipeline general del sistema]{Pipeline general del sistema: entrenamiento offline sobre CICIDS2017 e inferencia offline en la Fase 2. Fuente: elaboración propia a partir de \texttt{src/load\_cicids2017.py}, \texttt{src/canonical\_schema.py}, \texttt{src/train\_rl\_defender.py} y \texttt{scripts/predict\_real\_traffic\_v2.py}.}
\label{fig:pipeline-sistema}
\end{figure}
...
La enumeración ordenada de las 76 características, junto con las columnas fuente de CICIDS2017 y de Flowmeter-py que alimentan cada posición, se presenta en el Anexo~\ref{cap:anexo-esquema-canonico}. Ese anexo también explicita la correspondencia entre las posiciones 1--76 del vector de valores y las posiciones 77--152 de la máscara.

\begin{table}[htbp]
\centering
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{p{4.1cm}cp{7.2cm}}
\toprule
...
\bottomrule
\end{tabular}
\caption[Agrupación lógica de las 76 características canónicas]{Agrupación lógica de las 76 características canónicas de flujo. Todos los grupos se fusionan en un único vector plano antes de la entrada al modelo. Fuente: elaboración propia.}
...
\begin{itemize}
    \item $m_i = 1$ cuando existe una columna fuente mapeada para la característica;
    \item $m_i = 0$ cuando esa columna fuente no está disponible en el extractor.
\end{itemize}
...
La Figura~\ref{fig:vector-observacion} muestra la misma construcción desde el punto de vista de la entrada al modelo: columnas del extractor, mapeo al contrato canónico, 76 valores de características, 76 indicadores de presencia y vector final de 152 dimensiones.

\begin{figure}[htbp]
\centering
\resizebox{0.9\textwidth}{!}{\input{figuras/f4_vector_observacion}}
\caption[Construcción del vector de observación]{Construcción del vector de observación canónico de 152 dimensiones. Fuente: elaboración propia a partir de \texttt{src/canonical\_schema.py}.}
\label{fig:vector-observacion}
\end{figure}
...
\bottomrule
\end{tabular}
\caption[Matriz de recompensa del entorno del defensor binario]{Matriz de recompensa por defecto para el entorno del defensor binario. Los valores son supuestos experimentales registrados con cada ejecución. Fuente: elaboración propia a partir de \texttt{src/rl\_defender\_env.py}.}
\label{tab:reward-matrix}
\end{table}

La Figura~\ref{fig:agente-entorno} representa el bucle agente--entorno usado en esta formulación. Aunque se utiliza una interfaz de RL estándar, la recompensa depende únicamente de la acción actual y de la etiqueta real del flujo evaluado; con $\gamma = 0$, el objetivo aprendido se reduce a la recompensa inmediata.

\begin{figure}[htbp]
\centering
\resizebox{0.9\textwidth}{!}{\input{figuras/f3_agente_entorno}}
\caption[Interacción agente--entorno]{Interacción agente--entorno y matriz de recompensa de la decisión \texttt{PERMIT}/\texttt{BLOCK}. Fuente: elaboración propia a partir de \texttt{src/rl\_defender\_env.py} y del perfil \texttt{main-v1}.}
\label{fig:agente-entorno}
\end{figure}
...
La arquitectura de red del perfil \texttt{main-v1} se resume en la Figura~\ref{fig:arquitectura-qrdqn}: una entrada de 152 dimensiones, tres capas densas ocultas y una salida distribucional de 200 cuantiles por cada una de las dos acciones.

\begin{figure}[htbp]
\centering
\resizebox{\textwidth}{!}{\input{figuras/f5_arquitectura_qrdqn}}
\caption[Arquitectura QRDQN del perfil main-v1]{Arquitectura QRDQN del perfil congelado \texttt{main-v1}: observación canónica, capas densas, cuantiles por acción y decisión final por media de cuantiles. Fuente: elaboración propia a partir del perfil actual y del artefacto histórico coherente con esa arquitectura.}
\label{fig:arquitectura-qrdqn}
\end{figure}
...
Los hiperparámetros ($\gamma = 0$, arquitectura \texttt{[1024, 1024, 512]}, $200$ cuantiles, tasa de aprendizaje $5\times10^{-5}$, $20$ pasos de gradiente y $3\,000\,000$ de pasos para el perfil completo) constituyen el perfil fijo \texttt{main-v1}, establecido manualmente y congelado antes de la campaña final. No proceden de una búsqueda automática. El repositorio conserva una sonda Optuna exploratoria cuyo espacio no contiene esta combinación; por tanto, el perfil no debe presentarse como un óptimo seleccionado por esa sonda.

El listado completo de valores congelados se recoge en la Tabla~\ref{tab:hiperparametros-main}. La MAIN histórica comparte estos valores relevantes, pero no sustituye la ejecución fresca exigida por la campaña.

\begin{table}[htbp]
\centering
\small
\begin{tabular}{p{4.5cm}p{2.8cm}p{4.5cm}}
\toprule
...
Semilla de partición & 42 & \texttt{split\_seed}; fija la división de datos \\
Semilla del modelo & 42 & \texttt{model\_seed}; controla aprendizaje e inicialización \\
\bottomrule
\end{tabular}
\caption[Hiperparámetros congelados del perfil main-v1]{Hiperparámetros congelados del perfil \texttt{main-v1}. Perfil fijado a mano, no procedente de la sonda Optuna. Fuente: especificación de campaña y perfil mantenido en el repositorio.}
...
Cada ejecución produce un \texttt{RUN\_ID} único y autocontenido. Los trabajos de validación e inferencia generan artefactos independientes que enlazan la ejecución fuente y la especificación. Al cerrar la campaña, el exportador copia cada ejecución física sellada y genera un paquete comprimido con suma de comprobación; los puntos declarados como alias reutilizan la misma procedencia sin duplicar binarios. Git conserva la especificación, índices y evidencia ligera revisable, mientras que Git LFS se reserva para binarios rastreados que lo requieran.

\subsection{Controles de reproducibilidad}
\texttt{split\_seed} gobierna la selección y partición de filas; \texttt{model\_seed} se propaga a \texttt{numpy}, \texttt{random}, \texttt{torch}, el entorno y el modelo. Cada punto conserva ambos valores. El estudio de semillas mantiene fijo \texttt{split\_seed=42} y cambia \texttt{model\_seed=42--46}, con un directorio por ejecución. Estos controles reducen ambigüedad, aunque no eliminan todo el indeterminismo de kernels GPU; el entorno real y sus versiones se capturan en \texttt{environment.json}.
