...
\caption[Interacción agente--entorno]{Interacción agente--entorno y matriz de recompensa de la decisión \texttt{PERMIT}/\texttt{BLOCK}. Fuente: elaboración propia a partir de \texttt{src/rl\_defender\_env.py} y de la configuración MAIN.}
...
La arquitectura de red concreta de la ejecución MAIN se resume en la Figura~\ref{fig:arquitectura-qrdqn}: una entrada de 152 dimensiones, tres capas densas ocultas y una salida distribucional de 200 cuantiles por cada una de las dos acciones.

\begin{figure}[htbp]
\centering
\resizebox{\textwidth}{!}{\input{figuras/f5_arquitectura_qrdqn}}
\caption[Arquitectura QRDQN de la ejecución MAIN]{Arquitectura QRDQN de la ejecución MAIN: observación canónica, capas densas, cuantiles por acción y decisión final por media de cuantiles. Fuente: elaboración propia a partir de \texttt{config.json} de la ejecución MAIN.}
