...
Este capítulo fija los protocolos con los que se obtiene y se interpreta la evidencia experimental, desde la partición interna hasta la inferencia fuera de línea de la Fase~2. La secuencia evita extender las conclusiones más allá de los artefactos disponibles.

\section{Fases experimentales}
La metodología experimental se divide en dos fases. La primera fase comprueba el aprendizaje y la generalización interna sobre un banco de pruebas público (\emph{benchmark}). La segunda fase comprueba si el flujo de procesamiento (\emph{pipeline}) de preprocesamiento y modelo resultante es capaz de procesar tráfico capturado por el operador en un laboratorio doméstico cerrado bajo cambio de dominio.
...
La Figura~\ref{fig:composicion-dia} ofrece la misma información como comparación visual entre días. La lectura relevante para el protocolo no es solo el volumen total, sino la variación extrema de prevalencia de ataque entre capturas, que justifica no depender únicamente de una partición aleatoria por filas.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.92\textwidth]{f6_composicion_dia.pdf}
\caption[Composición visual por día de CICIDS2017]{Composición visual por día de CICIDS2017. Fuente: elaboración propia a partir de \texttt{memoria/figuras/data\_composicion\_dia.json}.}
...
En la ejecución MAIN, la estratificación mantiene prácticamente idéntica la prevalencia de ataque entre entrenamiento y prueba, como muestra la Figura~\ref{fig:balance-particion}. Esto permite atribuir los cambios de rendimiento entre entrenamiento y evaluación al modelo y al protocolo, no a una variación accidental de la proporción de clases en la partición aleatoria.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.78\textwidth]{f7_balance_particion.pdf}
\caption[Balance de clases en la partición aleatoria fija]{Balance de clases en la partición aleatoria fija de la ejecución MAIN. Fuente: elaboración propia a partir de \texttt{config.json} de la ejecución MAIN.}
\label{fig:balance-particion}
\end{figure}
...
\begin{figure}[htbp]
\centering
\figureplaceholder{Curvas de aprendizaje de QRDQN y Random Forest por presupuesto de entrenamiento}
\caption[Curvas de aprendizaje pendientes]{\placeholdercaption{Curvas de aprendizaje de QRDQN y Random Forest por presupuesto de entrenamiento.}}
\label{fig:placeholder-curvas-aprendizaje}
\end{figure}
...
La Figura~\ref{fig:escalera-validacion} sintetiza esta escalera de validación y sitúa cada peldaño junto al artefacto o resultado que lo respalda. La figura no añade métricas nuevas: reordena visualmente las cifras ya documentadas para hacer explícito el aumento progresivo de exigencia.

\begin{figure}[htbp]
\centering
\resizebox{\textwidth}{!}{\input{figuras/f16_escalera_validacion}}
\caption[Escalera de validación del proyecto]{Escalera de validación del proyecto, desde la evaluación directa hasta la inferencia de Fase 2. Fuente: elaboración propia a partir de los artefactos de validación y Fase 2 bajo \texttt{runs/}.}
...
{\raggedright\texttt{config.json}; \texttt{metrics.json}; \texttt{diagnostics.json}; muestra rastreada \texttt{predictions\_\hspace{0pt}head\_\hspace{0pt}10000.csv}; \texttt{predictions.csv.gz} completo, local y no rastreado\par} \\
\bottomrule
\end{tabular}
...
La Figura~\ref{fig:pipeline-fase2} resume el recorrido operativo de esa inferencia. La clave metodológica es que el escalador de MAIN se reutiliza sin reajuste y que las salidas son ficheros de predicción, métricas y diagnósticos: no existe actuación activa sobre la red.

\begin{figure}[htbp]
\centering
\resizebox{0.74\textwidth}{!}{\input{figuras/f17_pipeline_fase2}}
\caption[Pipeline de inferencia offline de la Fase 2]{Pipeline de inferencia offline de la Fase 2. Fuente: elaboración propia a partir de \texttt{scripts/predict\_real\_traffic\_v2.py}.}
