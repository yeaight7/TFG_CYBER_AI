# Graph Report - TFG_CYBER_AI  (2026-06-10)

## Corpus Check
- 79 files · ~157,469 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1468 nodes · 1620 edges · 96 communities (79 shown, 17 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 78 edges (avg confidence: 0.83)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `c97b734c`
...
Cohesion: 0.06
Nodes (37): CanonicalResult dataclass, FEATURES_CANON (76 canonical network flow features), NSL_KDD_TO_CANON mapping dict (partial), map_to_canonical function, Missingness Mask Design (observation = features + mask), load_nsl_kdd_binary function, batched_predict in v1, load_model (QRDQN/DQN fallback) in v1 (+29 more)
...
Cohesion: 0.07
Nodes (36): evaluate_random_forest function, baseline_random_forest main (3-sweep eval), train_random_forest function, CICIDS2017_TO_CANON mapping dict, Classification-as-RL / Contextual Bandit Equivalence, DummyVecEnv + Monitor Wrappers, Episode Mechanics: train shuffle / test deterministic, _prepare_cicids_features internal function (+28 more)
...
Cohesion: 0.17
Nodes (21): collect_environment_metadata(), evaluate_model(), main(), make_env_fn(), _package_version(), parse_args(), train_rl_defender.py — Entrenamiento de agente defensor RL sobre CICIDS2017., Devuelve una función creadora de entornos para DummyVecEnv. (+13 more)
...
Cohesion: 0.11
Nodes (21): Artifacts, code:bash (git clone https://github.com/yeaight7/TFG_CYBER_AI.git), code:text (models/<RUN_ID>.zip), code:text (runs/cicids2017/<RUN_ID>/), code:bash (tar -czf "<RUN_ID>_bundle.tar.gz" \), code:bash (python src/validate_checks.py \), code:text (datasets/CICIDS2017/*.csv), code:bash (python src/train_rl_defender.py \) (+13 more)
...
## Knowledge Gaps
- **902 isolated node(s):** `predict_real_traffic_v2.py — Robust Phase 2 offline inference pipeline.  Loads`, `If time columns look like seconds (median < 1), convert to microseconds.`, `Load QRDQN model, falling back to DQN if sb3_contrib is unavailable.`, `Run model.predict in batches to avoid OOM on large flow CSVs.`, `Compute z-score diagnostics on scaled features (first _N_CANON dims only).` (+897 more)
...
- **Why does `map_to_canonical()` connect `Community 22` to `Community 2`, `Community 5`?**
  _High betweenness centrality (0.026) - this node is a cross-community bridge._
- **Why does `main()` connect `Community 5` to `Community 22`?**
  _High betweenness centrality (0.024) - this node is a cross-community bridge._
- **Why does `_prepare_cicids_features()` connect `Community 2` to `Community 22`?**
