---
source_file: "src/load_cicids2017.py"
type: "code"
community: "CICIDS2017 Preprocessing"
location: "L226"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/CICIDS2017_Preprocessing
---

# _prepare_cicids_features()

## Connections
- [[Limpia CICIDS2017 y devuelve X, y y nombres de features.]] - `rationale_for` [EXTRACTED]
- [[_clean_rows()]] - `calls` [EXTRACTED]
- [[_coerce_numeric_features()]] - `calls` [EXTRACTED]
- [[_drop_identifier_like_columns()]] - `calls` [EXTRACTED]
- [[_find_label_column()]] - `calls` [EXTRACTED]
- [[_load_and_process_csv_paths()]] - `calls` [EXTRACTED]
- [[load_cicids2017.py]] - `contains` [EXTRACTED]
- [[load_cicids2017_binary()]] - `calls` [EXTRACTED]
- [[map_to_canonical()]] - `calls` [INFERRED]
- [[test_prepare_cicids_features_binary_labels()]] - `calls` [INFERRED]

#graphify/code #graphify/EXTRACTED #community/CICIDS2017_Preprocessing