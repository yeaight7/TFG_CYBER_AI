...
The experimental methodology is divided into two phases. The first phase tests learning and internal generalization on a public benchmark. The second phase tests whether the resulting preprocessing and model pipeline can process traffic captured by the operator in a closed home lab under domain shift.
...
It is important to clarify the exact semantics of the mask in the current implementation. The mask is computed \emph{after} the cleaning step that replaces non-finite values with zero (\texttt{src/load\_cicids2017.py}: $\pm\infty \to$ NaN $\to 0$), so that $m_i = 1$ whenever the canonical feature has a mapped source column. Because the CICIDS2017 $\to$ canonical mapping covers all 76 features, on native CICIDS2017 data the mask is \textbf{constant and equal to 1} across all rows: it encodes \emph{source-column presence}, not row-by-row value missingness. The mask only becomes informative in cross-domain or Phase 2 laboratory inference, where some source columns are unavailable and their mask value remains 0; a historical mapping like NSL-KDD, for example, only covers 3 of the 76 canonical features. Thus, the 152-dimensional observation consists of 76 feature values + 76 presence flags.
...
\subsection{leave-one-CSV-out Split}
The strictest internal protocol is leave-one-CSV-out validation. Each fold holds out one official CICIDS2017 CSV as test data and trains on the remaining CSVs. The implemented validation script aggregates per-fold metrics including accuracy, balanced accuracy, attack precision, attack recall, attack F1, benign precision, benign recall, false-positive rate, false-negative rate, block rate, total reward, and time costs.
...
\subsection{Discount Factor $\gamma = 0$: One-Step Formulation}
In all runs of this thesis, including the official run, the discount factor is fixed at $\gamma = 0$ (value recorded in the run configuration artifact). The choice is deliberate and consistent with the dataset-as-environment formulation: since the environment is a static dataset and the agent's actions do not affect future states, there is no temporal dependency between flows that a positive discount factor could exploit.

With $\gamma = 0$, the temporal-difference error defined above reduces to
\[
    \delta_{ij} = r - \theta_i(s, a),
\]
meaning the bootstrapping term $\gamma\,\theta_j^-(s', a^*)$ drops out and the target network no longer contributes. Each learned quantile $\theta_i(s,a)$ therefore regresses toward the immediate reward of the decision, and QRDQN acts as a one-step distributional estimator: formally, a \emph{cost-sensitive distributional contextual bandit}, rather than a sequential Markov Decision Process.

This reduction does not make the choice of QRDQN empty. The distributional head still models the full distribution of the asymmetric reward around the \texttt{PERMIT}/\texttt{BLOCK} boundary---not just its mean---which provides information about value uncertainty under asymmetric cost. The difference from a supervised classifier is not the presence of sequential dynamics, which are absent here by design, but that learning is driven by values over an explicit cost matrix (with the false-negative penalty much higher than the false-positive penalty) rather than by cross-entropy over labels. The thesis does not claim sequential autonomy or multi-step adversarial response: as noted in the formulation limitations, those capabilities remain outside the implemented scope.
...
The hyperparameters of the official run ($\gamma = 0$, architecture \texttt{[1024, 1024, 512]}, $200$ quantiles, learning rate $5\times10^{-5}$, $20$ gradient steps, and $3\,000\,000$ training timesteps) constitute a \emph{hand-set fixed profile}, not the output of an automated search. The repository includes an Optuna optimization probe (\texttt{src/tune\_hparams.py}) for exploratory purposes, but its search space---$\gamma \in [0.95, 0.999]$, architectures in $\{[256,128], [512,256], [256,256]\}$, and gradient steps in $\{10,50,100\}$---\emph{does not contain} the official configuration. Therefore, this configuration could not have originated from that search and should not be interpreted as an Optuna-tuned optimum; it reflects a manual design consistent with the one-step formulation, and is frozen and verified by the project's tests.
...
\begin{enumerate}
    ...
    \item leave-one-CSV-out validation, which withholds each official CSV in turn;
    \item Phase 2 laboratory-flow inference, which tests the trained pipeline on traffic captured by the operator in a closed laboratory environment.
\end{enumerate}
